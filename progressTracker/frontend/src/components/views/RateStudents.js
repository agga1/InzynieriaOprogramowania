...
export class RateStudents extends Component {
    ...
    handleSubmit = (e) => {
        ...
        if(new_grade <= this.state.task.grade_max && new_grade >= this.state.task.grade_min){
            ...
            alert("Enter proper grade from range: ["+this.state.task.grade_min+", "+this.state.task.grade_max+"]!");
        }
    }

    updateGradesList(new_grade){
        let item = this.state.grades.filter(grade => grade.student==this.state.student_id).map(grade => grade);
        ...
        if(item[0] !== undefined){
            ...
            this.setState((state) => {
                return {grades: [...state.grades, {student:state.student_id, value:new_grade}]}
            }, () => this.handleCancel());
        }
    }
